#include "../../include/netforge/websocket/WebSocketSession.hpp"
...
namespace netforge
{
    ...
    {
        auto self = std::static_pointer_cast<WebSocketSession>(shared_from_this());

        boost::asio::post(strand_, [self, message = std::move(message)]() mutable {
            if (!self->running()) {
                return;
            }
    ...
    void WebSocketSession::send(std::vector<std::uint8_t> data)
    {
        auto self = std::static_pointer_cast<WebSocketSession>(shared_from_this());

        boost::asio::post(strand_, [self, data = std::move(data)]() mutable {
            if (!self->running()) {
                return;
            }
    ...
    void WebSocketSession::on_start()
    {
    ...
    void WebSocketSession::on_stop()
    {
        write_queue_.clear();
        on_disconnected();
    }

    void WebSocketSession::on_error(const boost::system::error_code& ec)
    {
    ...
    void WebSocketSession::do_write()
    {
        ...
        websocket_.async_write(boost::asio::buffer(message.data), boost::asio::bind_executor(strand_,
                [self](boost::system::error_code ec, std::size_t) {
                    ...
                    if (self->running() && !self->write_queue_.empty()) {
                        self->do_write();
                    }
                }
            )
